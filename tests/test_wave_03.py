...
def test_mark_complete_missing_task(client):
    # Act
    task_id = "1"
    response = client.patch(f"/tasks/{task_id}/mark_complete")
    response_body = response.get_json()
    ...
    # *****************************************************************
    assert response_body == {"message": f"Could not find a task with id {task_id}"}
...
def test_mark_incomplete_missing_task(client):
    # Act
    task_id = "1"
    response = client.patch(f"/tasks/{task_id}/mark_incomplete")
    response_body = response.get_json()
